...
attached using the replacement rules provided via the Style and Labeled options. \n
If you use FCLoopIntegralToGraph for visualizing loop integrals, the first two \
entries of its output can be used as the input for FCLoopAddEdgeTags, e.g. \
FCLoopAddEdgeTags[FCLoopIntegralToGraph[FAD[p, p - k], {p}][[1 ;; 2]]]. \n
If you just want to plot the obtained graph, it is easier to process the output \
of FCLoopIntegralToGraph directly with FCLoopGraphPlot, which internally uses \
FCLoopAddEdgeTags.";
