...
(* :Title: AsymptoticExpansion-tad2LxFx111x122xxEp999x					*)
...
(* :Summary: Asymptotic expansion of a 2L tadpole integral 
			tad2LxFx111x122xxEp999x in m2^2/m1^2.                            *)
...
If[ $FrontEnd === Null,
		...
		Print["Asymptotic expansion of a 2L tadpole integral tad2LxFx111x122xxEp999x in m2^2/m1^2."];
];
...
{int,topo}={GLI[tad2LxFx111x122xxEp999x,{1,1,1}],FCTopology[tad2LxFx111x122xxEp999x,{FAD[{k1,m1}],FAD[{k2,m2}],FAD[{k1-k2,m2}]},{k1,k2},{},{},{}]}
id=topo[[1]]
...
regN=1;
scaling[regN]={m1->la^0 m1, m2->la^1 m2, k1->la^1 k1,k2->la^1 k2}
momShifts[regN]={};
...
regN=2;
scaling[regN]={m1->la^0 m1, m2->la^1 m2, k1->la^1 k1,k2->la^0 k2}
momShifts[regN]={k1->k1+k2};
propsR[regN]=ReplaceAll[{propsFC,propsAsy},momShifts[regN]];
topoR[regN]=FCReplaceMomenta[topo,momShifts[regN]];
{ExpandAll[(1/propsR[regN][[2]])],asyFinal}
...
Table[topoWithScaling[id,i]=FCLoopAddScalingParameter[topoR[i],la,scaling[i]],{i,1,4}]
...
(*v1=(1.*(24.5 + 1.*m2^2))/ep^2 - (9.2*^-7*(1.2739042891304348*^8 - 3.2608695652173916*^6*m2^2 + 1.*m2^4 + 4.3478260869565215*^6*m2^2*Log[m2]))/ep - 
 0.081632*(-5665.328137617601 + 239.79370700215603*m2^2 + 3.0111239464915722*m2^4 - 92.6999362994904*m2^2*Log[m2] - 3.3918458447667583*m2^4*Log[m2] - 49.00039200313603*m2^2*Log[m2]^2 + 1.*m2^4*Log[m2]^2) + 
 0.163264*ep*(-8105.703983486869 + 485.312677871423*m2^2 + 2.4495983192865545*m2^4 - 108.64430835946688*m2^2*Log[m2] + 0.45321810074480595*m2^4*Log[m2] - 46.3499681497452*m2^2*Log[m2]^2 - 2.6959155723245787*m2^4*Log[m2]^2 - 
   16.333464021462174*m2^2*Log[m2]^3 + 1.*m2^4*Log[m2]^3)*)
