...
$$\text{FeynCalc }\;\text{10.2.0 (dev version, 2026-05-18 15:37:55 +02:00, 8de568f1). For help, use the }\underline{\text{online} \;\text{documentation},}\;\text{ visit the }\underline{\text{forum}}\;\text{ and have a look at the supplied }\underline{\text{examples}.}\;\text{ The PDF-version of the manual can be downloaded }\underline{\text{here}.}$$
...
$$\{\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology1/job.yaml},\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology2/job.yaml}\}$$
...
$$\{\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology1/KiraLoopIntegrals},\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology2/KiraLoopIntegrals}\}$$

$$\left(
\begin{array}{cc}
 \;\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology1/config/integralfamilies.yaml} & \;\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology1/config/kinematics.yaml} \\
 \;\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology2/config/integralfamilies.yaml} & \;\text{/tmp/Reduction-quarkSE\$Amp-1L/fcPFRTopology2/config/kinematics.yaml} \\
\end{array}
\right)$$
...
$$\frac{i \;\text{as4} C_F \xi _{\text{G}} \delta _{\text{Col1}\;\text{Col2}} \gamma \cdot p}{\text{ep}}-\frac{i \;\text{as4} \;\text{mq} C_F \left(\xi _{\text{G}}+3\right) \delta _{\text{Col1}\;\text{Col2}}}{\text{ep}}$$
...
$$\left\{\text{rc}(\text{delZm},1)\to -\frac{3 C_F}{\text{ep}},\text{rc}(\text{delZpsi},1)\to -\frac{C_F \xi _{\text{G}}}{\text{ep}}\right\}$$
...
$$\left\{\text{denHead}\left(\frac{1}{(k^2+i \eta )}\right)\to \frac{1}{(k^2-\text{mxt}^2+i \eta )},\text{denHead}\left(\frac{1}{((k-p)^2+i \eta )}\right)\to \frac{1}{((k-p)^2-\text{mxt}^2+i \eta )},\text{denHead}\left(\frac{1}{((k-p)^2-\text{mq}^2+i \eta )}\right)\to \frac{1}{((k-p)^2-\text{mq}^2+i \eta )}\right\}$$
...
$$\{\text{/tmp/Reduction-gluonSE\$Amp-1L/gluonSEtopo1/job.yaml},\text{/tmp/Reduction-gluonSE\$Amp-1L/gluonSEtopo2/job.yaml}\}$$
...
$$\{\text{/tmp/Reduction-gluonSE\$Amp-1L/gluonSEtopo1/KiraLoopIntegrals},\text{/tmp/Reduction-gluonSE\$Amp-1L/gluonSEtopo2/KiraLoopIntegrals}\}$$
...
\begin{array}{cc}
 ...
 \;\text{/tmp/Reduction-gluonSE\$Amp-1L/gluonSEtopo2/config/integralfamilies.yaml} & \;\text{/tmp/Reduction-gluonSE\$Amp-1L/gluonSEtopo2/config/kinematics.yaml} \\
\end{array}
\right)$$
...
$$\left(
\begin{array}{cc}
 G^{\text{gluonSEtopo2}}(1)\to G^{\text{tad1Lv1}}(1) & G^{\text{gluonSEtopo1}}(1)\to G^{\text{tad1Lv2}}(1) \\
 G^{\text{tad1Lv1}}(1) & G^{\text{tad1Lv2}}(1) \\
\end{array}
\right)$$
...
$$\left\{\text{rc}(\text{delZA},1)\to \frac{-3 C_A \xi _{\text{G}}+13 C_A-4 N_f}{6 \;\text{ep}},\text{rc}(\text{delZAmxt},1)\to -\frac{C_A \left(3 \xi _{\text{G}}+1\right)}{8 \;\text{ep}},\text{rc}(\text{delZxi},1)\to \frac{-3 C_A \xi _{\text{G}}+13 C_A-4 N_f}{6 \;\text{ep}}\right\}$$
...
$$\text{FCLoopRewriteOverdeterminedTopologies: }\;\text{Generated }14\text{ new topologies through partial fractioning.}$$
...
$$\{\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology1/job.yaml},\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology2/job.yaml}\}$$
...
$$\{\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology1/KiraLoopIntegrals},\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology2/KiraLoopIntegrals}\}$$

$$\left(
\begin{array}{cc}
 \;\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology1/config/integralfamilies.yaml} & \;\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology1/config/kinematics.yaml} \\
 \;\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology2/config/integralfamilies.yaml} & \;\text{/tmp/Reduction-quarkGluonVTX\$Amp-1L/fcPFRTopology2/config/kinematics.yaml} \\
\end{array}
\right)$$
...
$$\left\{\text{Zm}\to 1-\frac{3 \;\text{as4} C_F}{\text{ep}},\text{Zpsi}\to 1-\frac{\text{as4} C_F \xi _{\text{G}}}{\text{ep}},\text{ZA}\to \frac{\text{as4} \left(-3 C_A \xi _{\text{G}}+13 C_A-4 N_f\right)}{6 \;\text{ep}}+1,\text{ZAmxt}\to 1-\frac{\text{as4} C_A \left(3 \xi _{\text{G}}+1\right)}{8 \;\text{ep}},\text{Zu}\to \frac{\text{as4} C_A \left(3-\xi _{\text{G}}\right)}{4 \;\text{ep}}+1,\text{Zumxt}\to 1,\text{Zg}\to 1-\frac{\text{as4} \left(11 C_A-2 N_f\right)}{6 \;\text{ep}},\text{Zxi}\to \frac{\text{as4} \left(-3 C_A \xi _{\text{G}}+13 C_A-4 N_f\right)}{6 \;\text{ep}}+1\right\}$$
...
$$\text{$\backslash $tCPU Time used: }42.837\text{ s.}$$