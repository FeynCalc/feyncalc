...
* Added ToSFAD, a function that converts FADs and PropagatorDenominators to SFADs and StandardPropagatorDenominators. (f300d7f6)
...
* Added new option Log to SimplifyPolyLog. When set to True, Logs will not be simplified. (8d750492)
...
* Fixed a bug with wrong propagator powers reported by FCLoopBasisExtract for SFADs and CFADs. (3f2b6e3b)
...
* Introduced initial support for StandardPropagatorDenominator and CSAD that allow to write down more generic covariant propagators propagators than currently possible with FAD. (b5fd4003)
...
* Improved typesetting of FADs so that now objects like FAD[p,Dimension->4] or FAD[{p+q,m}, Dimension->D-4] will have the momenta displayed with the correct bars and hats. This way now one can immediately see the dimension of a FAD. (30542a78)
...
* Fixed inconsistent behavior of FDS. If a loop integral factorizes into a product of loop integtrals, FDS can often detect that and give the result in such a way, that a single FAD is rewritten as a product of FADs. This behavior is controlled by the option FeynAmpDenominatorCombine. The default value is False (to be compatible with the old behavior). Setting this option to True activates the factorization. (8a7553f4)
...
* Added a unit test for Dirac trace. (524232d1)
...
* Improved FCFAConvert to handle FeynArts models with SUNDs in the Feynman rules (thanks to A. Hayreter). (59002150)
...
* Improved Collect2 so that it can now thread over (lists of) replacement rules. This is particulary useful when applying Collect2 to the output of Solve. (5c334b7d)
...
* Added new rules to SimplifyPolyLog. Those are useful when the argument appears as a square root. The relevant simlifications can be turned off via the Sqrt option. (0bc92d2a)
...
* Added new options Numerator and FactoringDenominator to Collect2. Those are meant for cases when Collect2 is used on fractions, i.e. Collect2[(....)/(....),...]. With Numeratore->True Collect2 will be applied only to the numerator but not the denominator. FactoringDenominator allows to specify a function that will applied to the denominator, e.g. Factor2. (1e48d104)
...
* Renamed the PDEHead option in PropagatorDenominatorExplicit to Head. Also added a new option Denominator  that allows to wrap propagator denominators (after writing out FADs) in the given head. (51fc4da4)
...
* Added new option DiracTrace to DiracSimplify. When set to False, DiracSimplify will completely ignore all DiracTraces in the expression. (3a439bbe)
...
* Fixed a bug in the typesetting of FADs. (c809f4a4)
* Fixed a bug in the typesetting of TemporalPair. (fc044986)
* Added new option Subtract to FCHideEpsilon and FCShowEpsilon. This allows one to specify what will be abbreviated together with 1/Eps, i.e. we not limited to 1/Eps - EulerGamma + Log[4Pi] anymore. (0c70248a)
...
* Minor refactoring in TARCER.m. (4d59f98e)
...
* Fixed a bug with premature memoization in to TFI (thanks to Philipp Schicho) (88ea81c8)
...
* Added new objects for non-relativistic calculations. Initial FCE/FCE support is available. Also, Eps does not have the option Dimension anymore. (a82bb76e)
...
* Added an example for calculations with the Euler-Heisenberg Lagrangian ( the model is generated using FeynArts). (d217744e)
