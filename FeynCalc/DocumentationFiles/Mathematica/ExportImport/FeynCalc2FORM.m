...
(* FeynCalc2FORM *)
(* ::Text:: *)
(*`FeynCalc2FORM[exp]` displays `exp` in `FORM` syntax. `FeynCalc2FORM[file, x]` writes `x` in FORM syntax to a file.   FeynCalc2FORM[file, x == y] writes $x=y$ to a file in FORM syntax.The capabilities of this function are very limited, so that you should not expect it to easily handle large and compicated expressions.*)
