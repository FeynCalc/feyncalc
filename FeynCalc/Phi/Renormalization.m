...
Options[FeynmanIntegral] = {SmallEpsilon -> SmallEpsilon,
			FCIntegrate -> IntegrateHeld};
Options[VeltmanDelta] :=
	{FixPoint -> 0, TaylorOrder -> 2};
Options[DimensionExpand] = {Dimension -> SpaceTimeDimensions, FixPoint -> 4,
			TaylorOrder -> 1};
Options[ExpandGammas] = {FixPoint -> 0, TaylorOrder -> 2, Dimension -> D};
Options[VeltmanExpand] = {(*Dimension -> D, MassScale -> ScaleMu,
			...
			D0Evaluation -> "none", FCIntegrate -> IntegrateHeld,
			...
			Masses -> {ParticleMass[Pion, RenormalizationState[0]],
					...
					ParticleMass[Pion, RenormalizationState[0]],
					ParticleMass[Pion, RenormalizationState[0]]}};
Options[LeutwylerLambda] = {Dimension -> D,
...
Options[LeutwylerJBar] = {LeutwylerJBarEvaluation -> "none",
			ExplicitLeutwylerSigma -> False};
Options[Renormalize] = {SUNN -> 2, InfinityFactor -> LeutwylerLambda[],
			RenormalizationCoefficientFunction[CouplingConstant[lag_, n_, ___]] :>
				RenormalizationCoefficients[lag][[n]]};
Options[FeynmanParameterize] = {Dimension -> D,
			FCIntegrate ->
			(DOT[Integratedx@@#2, #1] &),
...
DenominatorD[m10_, m20_, q0_, x_, opts___] :=
	m10*x + m20*(1 - x) - q0*x*(1 - x) -
		I*(SmallEpsilon /. Flatten[{opts}] /. Options[FeynmanIntegral]);
...
FeynmanIntegral[p10_, m10_, m20_, n1_, n2_, d_, d1_,
	...
	IntegrateHeld -> (FCIntegrate /. Flatten[{opts}] /.
...
VeltmanB0[p10_, m10_, m20_,
	...
	iib0 =(*-*)
		1/(I*Pi^2)*(MassScale /. Flatten[{opts}] /.
			...
			(*m20, m10,*)m10, m20, 1,
				1, bdi, dedim,
				FCIntegrate -> IntegrateHeld, opts] /.
...
LeutwylerLambda[opts___Rule | opts___List] /;
	...
	1/(16*Pi^2)*(MassScale /. Flatten[{opts}] /.
					Options[LeutwylerLambda])^((Dimension /. Flatten[{opts}] /.
							Options[LeutwylerLambda]) -
						4)*(1/((Dimension /. Flatten[{opts}] /. Options[LeutwylerLambda]) -
...
s3[a_, b_, c_, yy0_, opts___] :=
	...
	If[ Im[imzero1] === 0,
		If[ (imzero1 < 1 && imzero1 > 0),
			FCPrint[1, "Warning : Integral decomposition being used outside range of \
validity - ", imzero1]
		]
	];
	If[ Im[imzero2] === 0,
...
llaa[i_, j_,
			moms__, {masses__}] :=
	-ll[i, j, moms, {masses}]*aa[i, moms, {masses}]*
...
FeynmanParameterize[input1_, rest__, opts___Rule|opts___List] :=
	Block[ {a,ex,ddiimm,res,momentabacklist,li,fps,len},
		...
		momentabacklist = {};
		res = input1 /.
			{a_*ex_FeynAmpDenominator :>
							FeynmanParameter1[a*ex, rest, opts],
		ex_FeynAmpDenominator :>
							FeynmanParameter1[ex, rest, opts]} /.
		IntegrateHeld -> (FCIntegrate /. {opts} /.
...
privatefeyparamter[
		num___*ex_FeynAmpDenominator, kay_,
		opts___] :=
	(ddiimm =
	Dimension /. Flatten[{opts}] /. Options[FeynmanParameterize];
	Block[ {g, dimension = ddiimm,(* Fix because we now use
			...
			denom = FeynAmpDenominator[dum], nokay, count, coefofksquared,
		...
		g /: g[x_, _]^2 := dimension /;  ! NumberQ[x];
		g[x_, x_] :=
			dimension /;  ! NumberQ[x];
		...
		denominator = ((List @@ ex) //.
				...
				Momentum[mom_, ___] :> (momentabacklist =
							Union[momentabacklist,
								Cases[MomentumExpand[Momentum[mom]], Momentum[_], Infinity,
										Heads -> True] /. Momentum -> Sequence];
									mom);
...
over which we're integrating (kay). *)
		nokay =
					Times @@
						Map[If[ FreeQ[#1, kay],
								#1,
								(denom = Join[denom, FeynAmpDenominator[#1]] /. dum -> Sequence[];
								1)
							] &,
							denominator];
...
(* Expand each term in the denominator. *)
		denom = Map[Expand[#1 //. PropagatorDenominator[a_, b_] :> a^2 - b^2] & , denom,
				2];
...
(kay^2 + 2 ell kay + msquared). *)
		...
		numerator =
			...
			(lil = Unique[li];
			Pair[LorentzIndex[lil],Momentum[mom1]] Pair[LorentzIndex[lil],Momentum[mom2]])/.
					Pair[LorentzIndex[li_, ___],
							Momentum[mom_, ___]] :> (momentabacklist =
								Union[momentabacklist,
									Cases[MomentumExpand[Momentum[mom]], Momentum[_], Infinity,
											Heads -> True] /. Momentum -> Sequence];
		...
		integrators] /.
		g[a_, b_] :> Pair[LorentzIndex[a], LorentzIndex[b]]
	]);
...
VeltmanExpand[amp_, opts___?OptionQ] :=
	...
	res = amp /. ParticleMass[a__]^2 -> ParticleMass[a]^2 /.
				{B0[p10_, m10_, m20_] :> VeltmanB0[p10, m10, m20, ##]& @@
						OptionsSelect[VeltmanB0, opts, Sequence @@
																					Select[Options[VeltmanExpand], FreeQ[#,FCIntegrate]&]],
					...
					If[ (OnMassShell /. Flatten[{opts}] /. Options[VeltmanExpand]),
						Table[Pair[Momentum[mv[irep], ___], Momentum[mv[irep], ___]] ->
						masses1[irep]^2, {irep, Length[(Masses /. Flatten[{opts}] /.
						Options[VeltmanExpand])]}],
						{}
					] /.
					IntegrateHeld -> (FCIntegrate /. Flatten[{opts}] /. Options[VeltmanExpand]) /.
...
Renormalize[m_, opts___] :=
	m /. CouplingConstant[a_[l_], n_, aaa___, RenormalizationState[0],
				b___] :> (CouplingConstant[a[l], n, aaa, RenormalizationState[1],
						b] + (RenormalizationCoefficientFunction[
										CouplingConstant[a[l], n]] /. Flatten[{opts}] /.
