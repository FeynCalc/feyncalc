...
dgammaRep[dim1_,dim2_]:= Which[
	...
	MatchQ[dim1,_Symbol-4] && dim1===dim2,
		OverscriptBox["\[Gamma]", "^"],
	True,
		SubscriptBox["\[Gamma]", ToBoxes[dim1]]
];
...
ExplicitLorentzIndex /:
	MakeBoxes[ ExplicitLorentzIndex[p_, dim_ : 4], TraditionalForm]:=
		If[ $LorentzIndices =!= True, ToBoxes[p,TraditionalForm],
...
MakeBoxes[FAD[a__], TraditionalForm]:=
	ToBoxes[1/ Apply[Times,Map[ff, {a}]], TraditionalForm];
...
MakeBoxes[f_. FeynAmpDenominator[a__], TraditionalForm ]:=
	(MakeBoxes[#,TraditionalForm]&)@@{f/ Apply[DOT, Map[( (#[[1]]/.Momentum[aa_,___]:>aa)^2 -
	#[[2]]^2 )&, {a}]]}
...
LorentzIndex /:
	MakeBoxes[ LorentzIndex[p_, dim_ : 4], TraditionalForm]:=
		...
		ToBoxes[dim, TraditionalForm]]]/; !MatchQ[p,$FCLorentzIndexSubHeads];
...
momentumRep[p_,dim_]:=
	Which[
		dim===4,
			OverscriptBox[ToBoxes[p], "_"],
		MatchQ[dim,_Symbol],
			ToBoxes[p],
		MatchQ[dim,_Symbol-4],
			OverscriptBox[ToBoxes[p], "^"],
		True,
			SubscriptBox[ToBoxes[p], ToBoxes[dim]]
];
...
Momentum /:
	MakeBoxes[ Momentum[ HighEnergyPhysics`qcd`OPEDelta`OPEDelta, _:4 ], TraditionalForm]:=
		Tbox[OPEDelta];
...
Momentum /:
	MakeBoxes[Momentum[(p:Subscript|Superscript)[x_,y_], dim_: 4], TraditionalForm] :=
		If[p===Subscript,
				SubscriptBox[TBox[Momentum[x, dim]], ToBoxes[y]],
				SuperscriptBox[TBox[Momentum[x, dim]], ToBoxes[y]]
		];
...
MakeBoxes[Pair[Momentum[a_, dim1_ : 4], Momentum[b_, dim2_ : 4]]^n_Integer?Positive,
	TraditionalForm]:=
...
Pair /:
	MakeBoxes[Pair[ Momentum[a_, dim_ : 4], Momentum[a_, dim_ : 4]],
	TraditionalForm]:=
		If [Head[a]===Plus,
...
MakeBoxes[Power[Pair[ Momentum[a_, dim_ : 4], Momentum[a_, dim_ : 4]],n_Integer?Positive],
TraditionalForm]:=
	If [Head[a]===Plus,
...
PartialD[c:OPEDelta..]:=
	PartialD @@ (Momentum /@ {c});
PartialD[x_LorentzIndex, y__LorentzIndex]:=
	DOT @@ Map[PartialD, {x, y}];
PartialD[x_Momentum, y__Momentum]:=
	DOT @@ Map[PartialD, {x, y}];
...
QuantumField /:
	MakeBoxes[ QuantumField[PartialD[pa_], a_, lori: (LorentzIndex | ExplicitLorentzIndex)[_,_ : 4]...,
	suni:SUNIndex[_]...], TraditionalForm]:=
		RowBox[{SubscriptBox["\[PartialD]", Tbox[pa]],
		SubsuperscriptBox[Tbox[a], Tbox[lori], Tbox[suni]]}];

QuantumField /:
	MakeBoxes[ QuantumField[PartialD[pa_]^m_, a_,  lori: (LorentzIndex | ExplicitLorentzIndex)[_,_ : 4]...,
	suni:SUNIndex[_]...],
	TraditionalForm]:=
...
QuantumField /:
	MakeBoxes[ QuantumField[pa__PartialD, a_, lori: Momentum[_,_ : 4]..., suni:SUNIndex[_]...],
	TraditionalForm]:=
		RowBox[{TBox[pa], SubsuperscriptBox[Tbox[a], Tbox[lori], Tbox[suni]]}];

QuantumField /:
	MakeBoxes[ QuantumField[pa__PartialD, a_, lori: (LorentzIndex | ExplicitLorentzIndex)[_,_ : 4]...,
	suni:SUNIndex[_]...], TraditionalForm]:=
		RowBox[{TBox[pa], SubsuperscriptBox[Tbox[a], Tbox[lori], Tbox[suni]]}];
...
HoldPattern[SUNF[i_,j_,k_,l_, OptionsPattern[]]]:=
	(With[{sui=Unique["Global`s"]}, SUNF[i,j,SUNIndex[sui]] *
	SUNF[SUNIndex[sui],k,l]])/; OptionValue[Explicit];
