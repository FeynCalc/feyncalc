...
sg[z_] :=
	z /. gti->gtis;
gtis[z__] :=
	gtis[z] = gti[z]/.gtirules/.D->4+Epsilon;
(*
...
PP :=
	PP = SPD[Momentum /. Options[GTI], Momentum /. Options[GTI]];
Dp :=
	Dp = SOD[Momentum /. Options[GTI]];
...
ps[0][z_] :=
	z;
ps[m_][z_] :=
	eExpand[PowerSimplify[(-1)^m] z] /; m =!= 0;
...
*)
FCPrint[1,"SimplifyGTI.m loaded"];
End[]
