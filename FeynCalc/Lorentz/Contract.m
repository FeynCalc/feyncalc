...
*)
	...
	Contract3[x_ /; (Head[x] =!= Times) && Head[x] =!= Plus] :=
...
Contract[a_, b_ /;Head[b] =!= Rule, c_ /; Head[c] =!= Rule, ops___Rule] :=
	Block[ {lc, new = 0, i},
		FCPrint[2,"longcontract1"];
		lc = Contract[b, c, ops];
		FCPrint[2,"longcontract1done"];
...
*)
	...
	contract22[xx_, yy_Pair] :=
		contra3a[xx, {yy}] /.  contra3a -> contra3b /.
		Pair -> pairsave /.  contra3b -> contra3c /. pair2 -> Pair;
...
(*epscondef*)
	...
	epscon/: epscon[a1_,a2_,a3_,a4_,OptionsPattern[Eps]] epscon[b1_,b2_,b3_,b4_,OptionsPattern[Eps]] :=
			( - ($LeviCivitaSign)^2 Det[{{PairContract[a1,b1],PairContract[a1,b2],PairContract[a1,b3],PairContract[a1,b4]},
					...
					{PairContract[a4,b1],PairContract[a4,b2],PairContract[a4,b3],PairContract[a4,b4]}}
				]//Expand
			)/.PairContract->Pair;                              (*epsevdef*)
...
*)
	contracT[x_,opt___Rule] :=
		Module[ { contractres = x,contractepsopt,
...
*)
			...
			contractres = contractres /. Pair->PairContract3 /.
										PairContract3 -> PairContract /.PairContract->Pair;
(*
			contractres = contractres /. Pair->PairContract3 /.
						PairContract3 -> ExpandScalarProduct/.
						Pair -> PairContract /.PairContract->Pair;
*)
(*
		If[!FreeQ[contractres, ScalarProduct],
			contractres = FeynCalcInternal[contractres];
			];
*)
			If[ schout =!= 0,
				If[ (contractepsopt===True) && (!FreeQ[contractres, Eps]) &&
					...
					If[ rename === True,
						contractres = doubleindex[contractres]
					]
				]
			];
	(*
