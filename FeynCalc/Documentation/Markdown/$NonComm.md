## $NonComm

`$NonComm` contains a list of all noncommutative heads present.

### See also

[Overview](Extra/FeynCalc.md), [NonCommQ](NonCommQ.md), [NonCommutative](NonCommutative.md).

### Examples

```mathematica
$NonComm
```

$$\{\text{DiracGamma},\text{DiracSigma},\text{GA},\text{GAD},\text{GAE},\text{GS},\text{GSD},\text{GSE},\text{GSLP},\text{GSLN},\text{GSLR},\text{GSLPD},\text{GSLND},\text{GSLRD},\text{GALP},\text{GALN},\text{GALR},\text{GALPD},\text{GALND},\text{GALRD},\text{LeftPartialD},\text{LeftRightPartialD},\text{LeftRightPartialD2},\text{LeftNablaD},\text{LeftRightNablaD},\text{LeftRightNablaD2},\text{FCPartialD},\text{QuantumField},\text{RightPartialD},\text{RightNablaD},\text{Spinor},\text{SpinorU},\text{SpinorUBar},\text{SpinorV},\text{SpinorVBar},\text{SUNT},\text{PauliSigma},\text{PauliXi},\text{PauliEta},\text{PauliEtaC},\text{TGA},\text{CGA},\text{CGAD},\text{CGAE},\text{CGS},\text{CGSD},\text{CGSE},\text{SI},\text{SID},\text{SIE},\text{CSI},\text{CSID},\text{CSIE},\text{FeynCalc$\grave{ }$Package$\grave{ }$dotsimpHold},\text{FCChargeConjugateTransposed},\text{CovariantD},\text{FieldStrength},\text{QuarkGluonVertex},\text{QuarkPropagator},\text{ChiralityProjector},\text{Spinor},\text{DiracMatrix},\text{DiracSlash}\}$$