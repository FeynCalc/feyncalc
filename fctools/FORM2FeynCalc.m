...
dott[a_,b_] := Pair[Momentum[a,dim], Momentum[b,dim]];

Clear[holdplus];
SetAttributes[holdplus, HoldAll];
SetAttributes[holdplus2, HoldAll];
holdplus[z__] := ((holdplus2@@{z}) /. holdplus->Plus /.
...
myDot[n_Integer, m__] := n myDot[m];

Unprotect[Times, Plus];
ClearAttributes[Times, Orderless]; ClearAttributes[Plus, Orderless];
...
If[dim =!= 4, rr = ChangeDimension[rr, dim]];
...
(* or make a list *)
Clear[getformexpr];
getformexpr[si_List] := Block[{new="{", flag = False, s, res},
s = Append[si," "];
...
(* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ *)
If[$VeryVerbose > 0,WriteString["stdout", "FORM2FeynCalc | \n "]];
Null
