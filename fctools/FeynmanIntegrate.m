...
BeginPackage["HighEnergyPhysics`fctools`FeynmanIntegrate`",
   "HighEnergyPhysics`FeynCalc`"];

FeynmanIntegrate::usage = "FeynmanIntegrate[exp, moms, vars] \
...
Options[FeynmanIntegrate] = {NIntegrate -> False, Dimension -> D,
...
FeynmanIntegrate::"integrate"="There is a problem checking for \
integrability. Please check that your input is correct. Aborting.";

FeynmanIntegrate::"transf"="There is a problem transforming to new \
variables. Please check that your input is correct. Aborting.";

FeynmanIntegrate::"noatom"="Please use only atomic variables as Feynman \
parameters. Not `1`. Aborting.";

FeynmanIntegrate::"nodeltaresolv"="Could not resolve delta functions in \
...
factest[vars_List] :=
   Block[{tfac}, PolynomialQ[Denominator[#]*Numerator[#], vars] &&
...
FeynmanIntegrate[Dot[ints0:(Integratedx[_, _, _]..),
...
If[And @@ ((Head[#]===Times && !FreeQ[#,DeltaFunction[_],{1,1}] &&
  !FreeQ[DeltaFunction[_],#] || FreeQ[DeltaFunction[_],#])& /@ tc),
    ...
    If[And @@ ((Head[#]===Times && !FreeQ[#,DeltaFunction[_],{1,1}] &&
      ...
      ress=tcc,
      Message[FeynmanIntegrate::"nodeltaresolv",exp];Abort[]]
];
If[$VeryVerbose>=2,
    Print["Found ",Length[ress]," terms with ",
      Count[ress,DeltaFunction[_],Infinity]," DeltaFunctions"]];
Plus@@(FeynmanIntegrate1[Dot[ints0, #], moms0, vars0, opts]& /@ ress),
If[Head[exp]===Plus,
  FeynmanIntegrate1[Dot[ints0, #], moms0, vars0, opts]& /@ exp,
  FeynmanIntegrate1[Dot[ints0, exp], moms0, vars0, opts]],
FeynmanIntegrate1[Dot[ints0, exp], moms0, vars0, opts]
]
];


FeynmanIntegrate1[exp0_, moms0_List:dum, vars0_List:dum, opts___Rule] :=
   Block[{(*ruls, dumf, a, b, p, r, s, cou, sym, m, n, eps, gamma, kk, beta,
    ...
    DD = Dimension /. Flatten[{opts}] /. Options[FeynmanIntegrate],
    epsorders=EpsilonOrder /. Flatten[{opts}] /. Options[FeynmanIntegrate]},
      ...
      (
       ...
       If[$VeryVerbose>=1,
         Print["Term is divergent to order ", epsorder, " in Epsilon"]];
       ...
       If[vars0 === dum,
         ...
         varlimsout = {};
         If[$VeryVerbose>=3,
           Print["No subset of integration variables given. Using variables from Integratedx: ", varlims]],
         ...
         If[$VeryVerbose>=3,
           Print["Subset of integration variables given. Using variables: ", varlims]];
       ];

       If[AtomQ[#]=!=True, Message[FeynmanIntegrate::"noatom",#];Abort[]]&/@vars;
       ...
       (delcol=(
        ...
        If[Length[delcol[[1]]]>0&&Length[delcol[[2]]]>0,
          ...
          varlims = If[MatchQ[#[[1]],Alternatives@@newvars],
                      If[#[[1]]===newvars[[-1]],(*drop the last variable*)seq[],
                        ReplacePart[#,Plus@@delcol[[1]],3]],#]& /@ varlims /. seq -> Sequence;
          If[$VeryVerbose>=2,
            Print["Transforming to triangular coordinates, jacobian factor is now: ",
              transfac]];
          ...
          If[$VeryVerbose>=3,
            Print["Intermediate transformed expression ", res]];
          res,
          # ])&,

       exp];
       
       If[transfac===0,Message[FeynmanIntegrate::"transf"];Abort[]];

       vars = Complement[vars, varsdrop];
       varp = Alternatives @@ vars;
       If[$VeryVerbose>=1 && {exp}=!=expred,
         Print["DeltaFunction(s) encountered, eliminating ", varsdrop]];

       moms = If[moms0 === dum, {}, moms0];
       kk = Alternatives @@ moms;

       (*If so chosen, expand without blowing up too much:*)
       If[(Expand /. Flatten[{opts}] /. Options[FeynmanIntegrate]),
       ruls = {};
       If[$VeryVerbose>=2,Print["Starting expansion of expression size: ", LeafCount[exp]]];
       serie = Expand[dumf*transfac*(expred) /.
            Log[(a_?(!FreeQ[#, kk]&))*(b_?(FreeQ[#, kk]&))] :>
             Log[a] + Log[b] /.
             (cou = 0;
             a_?( (!NumberQ[#] && NumericQ1[#, Union[vars,(*Not having Epsilon here
             blows up things too much when considering xprime instead of x in the
             Itzykson&Zuber calculation*){(*Epsilon*)}]] &&
       ...
       serie = dumf*transfac*(expred)];
        ...
        If[$VeryVerbose>=2 && (Expand /. Flatten[{opts}] /. Options[FeynmanIntegrate]),
          Print["Finished expansion. Expression size: ", LeafCount[serie0],
                ". Length: ", Length[serie0]]];

       (*Simplify if so chosen*)
       If[(Simplify /. Flatten[{opts}] /. Options[FeynmanIntegrate]),
          If[$VeryVerbose>=2,Print["Starting simplification"];WriteString["stdout", "\n"]];
          serie1 = (If[$VeryVerbose>=2,WriteString["stdout", "."]];Simplify[#])& /@
            serie0;
       ...
       If[(Series /. Flatten[{opts}] /. Options[FeynmanIntegrate]),
       If[$VeryVerbose>=1,
       ...
       serie3 = serie2];
        ...
        If[Head[serie3] =!= Plus, serie4 = {serie3}, serie4 = List@@serie3];
        If[$VeryVerbose>=2 && (Expand /. Flatten[{opts}] /. Options[FeynmanIntegrate]),
          Print["Finished expansion. Expression size: ", LeafCount[serie3],
       ...
       Do[
          ...
          (int|nint|out|no)[1] :> 1;
           ...
           Which[
            ...
            seri = Append[seri,
              Dot[ Sequence@@((Integratedx@@##)& /@ varlimsout),
               (FCIntegrate /. Flatten[{opts}] /. Options[FeynmanIntegrate])[
           ...
           seri = Append[seri,
               Dot[ Sequence@@((Integratedx@@##)& /@ varlimsout),
               (FCNIntegrate /. Flatten[{opts}] /. Options[FeynmanIntegrate])[
           ...
           Message[FeynmanIntegrate::"integrate"];Abort[]
...
End[]; EndPackage[];
(* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ *)
If[$VeryVerbose > 0, WriteString["stdout", "FeynmanIntegrate | \n "]];
Null