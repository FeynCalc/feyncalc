...
BeginPackage["HighEnergyPhysics`fctools`RHI`",
...
(*
If[Head[new]===Plus,
   new = Select2[new,Epsilon],
   If[FreeQ[new,Epsilon], new = 0]
  ];
*)
new = Factor1[new];
...
(*
   MakeBoxes[RHI[{a_,b_,c_,d_,e_},{al_,be_,ga_,de_,ep_}] :=
     SubsuperscriptBox["I", TBox[al,be,ga,de,ep], TBox[a,b,c,d,e]];
            ];
   MakeBoxes[RHI[{x_,y_,z_,v_,w_}, {a_,b_,c_,d_,e_},
                 {al_,be_,ga_,de_,ep_} 
                ] :=
     SubsuperscriptBox["I", TBox[al,be,ga,de,ep],
                             RowBox[TBox[x,y,z,v,w], " ",TBox[a,b,c,d,e]]
                       ];
            ];
*)
